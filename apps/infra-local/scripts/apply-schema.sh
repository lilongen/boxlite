#!/usr/bin/env bash
# Load the merged schema (synthesized from apps/api TypeORM migrations) into
# the local boxlite-local-postgres box.
#
# The SQL file is regenerated by `make load-schema` (which runs
# scripts/build-all-in-one-sql.py just before this script). This script only
# loads whatever is already at sql/merged-schema.auto-gen.sql.
#
# When to run:
#   - First time after `make up` on a fresh data dir
#   - After `make wipe` (which clears the data dir)
#   - After apps/api migrations change (regenerate via `make load-schema`)
#
...
# Default to the generated merged schema; override with SCHEMA_SQL_FILE to load
# a different file.
SQL_FILE="${SCHEMA_SQL_FILE:-${SCRIPT_DIR}/../sql/merged-schema.auto-gen.sql}"

if [[ ! -f "${SQL_FILE}" ]]; then
  echo "ERR: ${SQL_FILE} not found." >&2
  echo "     Generate it first: python scripts/build-all-in-one-sql.py" >&2
  echo "     (or just run 'make load-schema', which regenerates it)." >&2
  exit 1
fi
